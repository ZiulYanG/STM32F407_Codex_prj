...
static StackType_t defaultTaskStack[512];
...
int main(void)
{
  ...
  /* The Bootloader deliberately hands off with PRIMASK set.  The Application
     owns the interrupt configuration from this point and must restore global
     interrupts before HAL_Init() starts the TIM7 HAL timebase. */
  __enable_irq();
